...
/*   Updated: 2024/12/12 14:32:53 by vsyutkin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
keeping it sorted from lowest to highest. */
int	ft_find_ra(t_list **list_a, int target)
{
	...
	if (target < find_min(*list_a) || target > find_max(*list_a))
		return (ft_find_rx(find_min(*list_a), *list_a));
	prev = find_max(*list_a);
	next = find_min(*list_a);
	temp = *list_a;
	while (temp)
	{
	...
	temp = *list_a;
	return (ft_find_rx(prev, *list_a));
}

/*How many rotate needs in list b to bring target to top of the stack,
keeping it sorted from highest to lowest. */
